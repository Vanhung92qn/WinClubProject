...
public class XocDiaKubetDaoImpl
    ...
    public List<TransactionXocDia> getLichSuXocDia(String username, int page) {
        ...
        iterable = db.getCollection("xoc_dia_kubet_transaction").find(conditions).sort(sortCondtions).skip(skipNumber).limit(10);
    ...
    public int countLichSuXocDia(String username) {
        MongoDatabase db = MongoDBConnectionFactory.getDB();
        MongoCollection<Document> collection = db.getCollection("xoc_dia_kubet_transaction");
        Document conditions = new Document();
        conditions.put("user_name", username);
