...
DB_BACKUP_PATH='/home/card-game/backup/dump'
MONGO_HOST='10.40.112.5'
MONGO_PORT='27017'
...
AUTH_ENABLED=1
MONGO_USER='admin'
MONGO_PASSWD='MgqzAtRymcxyNoFnkwX7sGUlmj0YQI'
...
docker exec 52c83c61f578 mongodump --host ${MONGO_HOST} --port ${MONGO_PORT} ${AUTH_PARAM} --out ${DB_BACKUP_PATH}/${TODAY}/
else
	echo "Running backup for selected databases"
	for DB_NAME in ${DATABASE_NAMES}
	do
docker exec 52c83c61f578 mongodump --host ${MONGO_HOST} --port ${MONGO_PORT} --db ${DB_NAME} ${AUTH_PARAM} --out ${DB_BACKUP_PATH}/${TODAY}/
	done
fi
