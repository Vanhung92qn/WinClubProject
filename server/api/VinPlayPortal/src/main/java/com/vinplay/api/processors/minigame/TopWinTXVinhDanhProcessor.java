...
public class TopWinTXVinhDanhProcessor
    ...
    public String execute(Param<HttpServletRequest> param) {
        ...
        if ((txType == null || txType.equals("1")) && typeVinhDanh != null) {
            ...
            try {
                ...
                //old mysql
//              response.setTotalMoneyStakesMine(Objects.equals(typeVinhDanh, BY_DAY) ? dao.getMoneyStakesTX(userCurrent) : dao.getMoneyStakesTXByMonth(userCurrent, Timestamp.from(Instant.from(getFirstDateOfMonth.withDayOfMonth(1).atStartOfDay(zoneId))), Timestamp.from(Instant.now())));
                //update new elk
                ...
                //test log
//                Long old = Objects.equals(typeVinhDanh, BY_DAY) ? dao.getMoneyStakesTX(userCurrent) : dao.getMoneyStakesTXByMonth(userCurrent, Timestamp.from(Instant.from(getFirstDateOfMonth.withDayOfMonth(1).atStartOfDay(zoneId))), Timestamp.from(Instant.now()));
//                sendLogToTele("old:" + old+" new:"+response.getTotalMoneyStakesMine());
                //
