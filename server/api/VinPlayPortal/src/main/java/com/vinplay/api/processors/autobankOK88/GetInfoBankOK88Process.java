...
public class GetInfoBankOK88Process implements BaseProcessor<HttpServletRequest, String> {
    public synchronized String execute(Param<HttpServletRequest> param) {
        try {
            HttpServletRequest request = (HttpServletRequest) param.get();
            String bank = request.getParameter("bank");
            bank = bank.toUpperCase();
            BankInfo ban = findBank(bank.trim());
            Gson gson = new Gson();
            if(ban == null){
    ...
    public BankInfo findBank(String bank){
        try {
