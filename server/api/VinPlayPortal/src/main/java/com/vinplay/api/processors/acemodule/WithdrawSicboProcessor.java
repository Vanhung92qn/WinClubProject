...
public class WithdrawSicboProcessor  implements BaseProcessor<HttpServletRequest, String> {
    ...
    private String getRequest(WithDrawRequestEntity depositRequest) throws IOException {
        ...
        Request request = new Request.Builder()
                .url("https://xingxing.bandoluuniem.net/sicbo/withdraw?brand=sunvin") //https://xingxing.bandoluuniem.net/sicbo/withdraw
                .method("POST", body)
                .addHeader("Content-Type", "application/json")
                .build();
        Response response = client.newCall(request).execute();
