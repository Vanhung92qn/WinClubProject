...
public class WithdrawBanCaProcessor implements BaseProcessor<HttpServletRequest, String> {
    ...
    private String getRequest(WithDrawRequestEntity depositRequest) throws IOException {
        ...
        Request request = new Request.Builder()
                .url("https://bangchiu.bandoluuniem.net/banca/withdraw?brand=sunvin")
                .method("POST", body)
                .addHeader("Content-Type", "application/json")
                .build();
        Response response = client.newCall(request).execute();
