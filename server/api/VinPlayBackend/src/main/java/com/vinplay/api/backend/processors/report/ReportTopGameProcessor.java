    ...
    public String execute(Param<HttpServletRequest> param) {
        ...
        block38:
        {
            ...
            try {
                ...
                if (startTime == null || endTime == null || startTime.isEmpty() || endTime.isEmpty() || action == null || action.isEmpty())
                    break block38;
                ...
                if (et.getTime() >= st.getTime()) {
                    if (st.getTime() >= currentDate.getTime()) {
                        for (Map.Entry entry : reportMap.entrySet()) {
                            ...
                            if (!actionname.equals(action) && (!action.equals("all") || !Consts.GAMES.contains(actionname)))
                                continue;
                        ...
                        for (Map.Entry entry4 : reportMap.entrySet()) {
                            ...
                            if (!actionname2.equals(action) && (!action.equals("all") || !Consts.GAMES.contains(actionname2)))
                                continue;
                ...
                if (action.equals("all")) {
                    for (TopCaoThu win : topUserWin) {
                        long money = otherService.getMoneyShootFishByNickname(sqlDateStart, sqlDateEnd, win.getNickname());
                        win.setMoneyWin(win.getMoneyWin() + money);
                    }
                    for (TopCaoThu lose : topUserLost) {
                        long money = otherService.getMoneyShootFishByNickname(sqlDateStart, sqlDateEnd, lose.getNickname());
                        lose.setMoneyWin(lose.getMoneyWin() + money);
                    }
