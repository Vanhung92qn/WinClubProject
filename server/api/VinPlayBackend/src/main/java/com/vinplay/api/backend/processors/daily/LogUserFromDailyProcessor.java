...
public class LogUserFromDailyProcessor implements BaseProcessor<HttpServletRequest, String> {
...
//                Collections.reverse(list_nap_rut);
            ...
            if(typeSearch.equals("mongo")) {
                ...
                Document conditions2 = new Document();
                if (!codeDaily.isEmpty()) {
                    conditions2.put("id_daily", codeDaily);
                    conditions2.put("nickName", "");
                } else {
                    return res.toJson();
                }
                //todo: check validate parameter
                long totalRows = db.getCollection("user_map_daily").count(conditions) - db.getCollection("user_map_daily").count(conditions2);
                ...
                iterable.forEach((Block<Document>) document -> {
                    ...
                    if (!nickNameUser.isEmpty()) {
                        ...
                        // GET TONG NAP RUT
                        HistoryTransResponse historyTrans = historyTransDao.getListTransByDay(nickNameUser, startTime, endTime);

                        // GET TONG NẠP RÚT YOU ĐẠI fLÝ
                        AgentServiceImpl service = new AgentServiceImpl();
                        long totalVinReceive = 0L;
                        long totalVinSend = 0L;
                        totalVinSend = service.totalMoneyVinSendFromAgentByStatus(nickNameUser, "1", startTime, endTime);
                        totalVinReceive = service.totalMoneyVinReceiveFromAgentByStatus(nickNameUser, "2", startTime, endTime);
                        if (historyTrans != null && historyTrans.getListTrans().size() > 0) {
                            logger.info("LogUserFromDailyProcessor off user: " + nickNameUser);
                            for (int i = 0; i < historyTrans.getListTrans().size(); i++) {
                                String status = historyTrans.getListTrans().get(i).trangthai.trim().toUpperCase();
                                if (status.equals(SUCCESS) || status.equals(APPROVED)) {
                                    String transMethod = historyTrans.getListTrans().get(i).hinhthucTrans;
                                    if (transMethod.equals(MOMO) || transMethod.equals(BANK) || transMethod.equals(CARD) || transMethod.equals(ONE_PAY)) {
                                        res.TotalIn += Long.parseLong(historyTrans.getListTrans().get(i).sotien);
                                        TotalInItem += Long.parseLong(historyTrans.getListTrans().get(i).sotien);
                                        records2.add(historyTrans.getListTrans().get(i));
                                    } else if (transMethod.equals(CARD_OUT) || transMethod.equals(BANK_OUT)) {
                                        res.TotalOut += Long.parseLong(historyTrans.getListTrans().get(i).sotien);
                                        TotalOutItem += Long.parseLong(historyTrans.getListTrans().get(i).sotien);
                                        records1.add(historyTrans.getListTrans().get(i));
                                    }
                                }
                            }
                        }
                        res.TotalIn += totalVinReceive; //Tổng Nạp All
                        res.TotalOut += -totalVinSend;   //Tổng Rút All
                        TotalInItem += totalVinReceive; // Tổng Nạp theo user
                        TotalOutItem += -totalVinSend;    //Tổng rút theo user
                        // GET LIST USER
                        UserWinLostResponse listUser = new UserWinLostResponse(nickNameUser, UserNameUser, CreateAtUser, TotalInItem, TotalOutItem);
