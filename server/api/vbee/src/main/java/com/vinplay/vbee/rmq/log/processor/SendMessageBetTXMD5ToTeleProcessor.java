...
public class SendMessageBetTXMD5ToTeleProcessor implements BaseProcessor<byte[], Boolean> {

    public Boolean execute(Param<byte[]> param) {
        BetTXMD5Message message = (BetTXMD5Message) BaseMessage.fromBytes(param.get());
        TelegramAlert.SendMessageBetTXMD5(message.getNickname(), message.getBetValue(), message.getBetSide(), message.getReferenceId(), message.getUserId());
        return true;
    }
