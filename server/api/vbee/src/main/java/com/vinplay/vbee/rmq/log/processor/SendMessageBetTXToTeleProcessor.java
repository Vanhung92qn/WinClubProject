...
public class SendMessageBetTXToTeleProcessor implements BaseProcessor<byte[], Boolean> {

    public Boolean execute(Param<byte[]> param) {
        BetTXMD5Message message = (BetTXMD5Message) BaseMessage.fromBytes(param.get());
        TelegramAlert.SendMessageBetTX(message.getNickname(), message.getBetValue(), message.getBetSide(), message.getReferenceId(), message.getUserId());
        return true;
    }
