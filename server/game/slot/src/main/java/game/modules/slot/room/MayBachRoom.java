...
//                            }
                            ...
                            for (AwardsOnLine entry2 : awardsOnLines) {
                                if ((entry2.getAward() == NDVAward.PENTA_NU_DIEP_VIEN || entry2.getAward() == NDVAward.QUADRA_NU_DIEP_VIEN || entry2.getAward() == NDVAward.TRIPLE_NU_DIEP_VIEN) && !forceNoHu)
                                    continue block4;
...
//                                    {
//                                        if ((entry2.getAward() == NDVAward.PENTA_NU_DIEP_VIEN || entry2.getAward() == NDVAward.QUADRA_NU_DIEP_VIEN || entry2.getAward() == NDVAward.TRIPLE_NU_DIEP_VIEN)) continue block4;
//                                    }
//                                    else if (ConfigGame.getIntValue(this.gameName + "_cho_bot_no_hu_100") == 1) // chỉ cho bot nổ hũ
//                                    {
//                                        if ((entry2.getAward() == NDVAward.PENTA_NU_DIEP_VIEN || entry2.getAward() == NDVAward.QUADRA_NU_DIEP_VIEN || entry2.getAward() == NDVAward.TRIPLE_NU_DIEP_VIEN) && !u.isBot()) continue block4;
//                                    }
...
//                                    this.fund = 0;
                                    ...
                                    if (forceJackpotByUser) {
                                        try {
                                            ...
                                            logger.error(" Reset cache MAYBACH - SonTinhThuyTinh error with : " + e.getMessage());
                                        }
                                    }
...
//                                if(moneyExchange != 0){
        ...
        if (forceJackpotByUser) {
            this.sendNotifyNoHu(username, (byte) 1, msg.prize,"MAYBACH");
        }
...
//            enoughPair = true;
            ...
            if (!updated) {
                ...
                if (moneyExchange > 0L) {
                    ...
                    MoneyResponse moneyRes = this.userService.updateMoney(username, totalPrizes, this.moneyTypeStr, "NuDiepVienVqFree", "Quay " + gn + " Free", "C\u01b0\u1ee3c: 0, Th\u1eafng: " + totalPrizes, 0L, (Long)null, TransType.NO_VIPPOINT);
                    if (moneyRes != null && moneyRes.isSuccess()) {
                        currentMoney = moneyRes.getCurrentMoney();
                    }
                }
