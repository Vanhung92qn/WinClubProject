...
<div class="wrapper">
    ...
    <div class="widget" style="background: #82d1ff; border-radius: 15px;">
        ...
        <div class="title">
            <h6 style="color: #001f0f;">Lịch sử nạp tiền qua Momo</h6>
        </div>
        <form class="list_filter form" action="<?php echo admin_url('report/rechargebymomo2') ?>" method="post">
            <div class="formRow">
                <table>
                    <tr>
                        <td>
            ...
            <div class="formRow">
                <table>
                    <tr>
                        ...
                        <td>
                            <input type="reset" onclick="window.location.href = '<?php echo admin_url('report/rechargebymomo2') ?>'; " value="Reset" class="basic" style="margin-left: 20px">
                        </td>
...
<script>
    ...
    function resultSearchTransction(stt, tid, nickname, money, bank, ip, status, description, time, updatetime, userApprove, sender) {
        ...
        //rs += "<td>" + updatetime + "</td>";
        // if (status == 1) {
        //     rs += "<td id=" + tid + "_action" + ">  <span class='label label-danger'><a style='color: white;' href=\"javascript: reject(" + tid + ")\">Từ chối</a></span><span class='label label-info'><a style='color: white;' href=\"javascript: approveMoMo(" + tid + ")\">Duyệt</a></span> </td>";
        // }
        // if (status == 2) {
        //     rs += "<td id=" + tid + "_action" + ">  <span class='label label-info'><a style='color: white;' href=\"javascript: approveMoMo(" + tid + ")\">Duyệt Lại</a></span> </td>";
        // } else {
        //     rs += "<td></td>"
        // }
    ...
    function approveMoMo(tid) {
        var moneyMomo = Number(document.getElementById(tid + "_money").value.replaceAll(',', '').replaceAll('.', ''));
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                if (result.ListTrans == "") {
                    ...
                    $("#resultsearch").html("");
                    var totalRecord = result.TotalTrans;
                    var totalPage = Math.floor(totalRecord / pageSize) * pageSize < totalRecord ? Math.floor(totalRecord / pageSize) + 1 : Math.floor(totalRecord / pageSize);
