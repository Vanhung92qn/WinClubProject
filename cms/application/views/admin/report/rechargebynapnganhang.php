...
<div class="wrapper">
    ...
    <div class="widget" style="background: #82d1ff; border-radius: 15px;">
        ...
        <form class="list_filter form" action="<?php echo admin_url('report/rechargebynapnganhang') ?>" method="post">
            <div class="formRow">
                <table>
                    <tr>
                        <td>
            ...
            <div class="formRow">
                <table>
                    <tr>
                        ...
                        <td>
                            <input type="reset" onclick="window.location.href = '<?php echo admin_url('report/rechargebynapnganhang') ?>'; " value="Reset" class="basic" style="margin-left: 20px">
                        </td>
...
<script>
    ...
    function reject(tid) {
        $.ajax({
            type: "POST",
            url: "<?php echo admin_url('report/updatedepositnganhangajax')?>",
            data: {
                transId: tid,
                type: 1,
				money: 0
	...
	function approveCodePay(tid) {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo admin_url('report/updatedepositnganhangmanual')?>",
            data: {
                transId: tid,
                type: 100,
				money: moneyCodePay
            ...
            success: function (result) {
                ...
                if (result.ListTrans == "") {
                    ...
                    $.each(result.ListTrans, function (index, value) {
                        // result += resultSearchTransction(stt, value.Id, value.Nickname, value.Amount, value.BankBrandName, value.Id, value.Status, value.Description, value.CreatedAt, value.UpdatedAt, value.UserApprove, value.UserSender);
                        // stt++;
						if(value.UserSender != 'CodePay' && value.UserSender != 'Momo') {
							result += resultSearchTransction(stt, value.Id, value.Nickname, value.Amount, value.BankBrandName, value.Id, value.Status, value.Description, value.CreatedAt, value.UpdatedAt, value.UserApprove, value.UserSender);
							stt++;
						}
                    });
