            ...
            <div class="formRow">
                <table>
                    <tr>
                        ...
                        <td><input type="text" style="margin-left: 20px;margin-bottom:-2px;width: 150px" id="txtmathe"
                                value="<?php echo $this->input->post('txtmathe') ?>" name="txtmathe"></td>
                        <td><label style="margin-left: 30px;margin-bottom:-2px;width: 100px">Mã serial:</label></td>
                        <td><input type="text" style="margin-left: 20px;margin-bottom:-2px;width: 150px" id="txtserial"
                                value="<?php echo $this->input->post('txtserial') ?>" name="txtserial"></td>
                    </tr>
                </table>
            </div>
            <div class="formRow">
                    ...
                    <tr>
                        ...
                        <td class="">
                            <select id="select_status" name="select_status"
                                ...
                                <option value="0"
                                    <?php if($this->input->post('select_status') == "0" ){echo "selected";} ?>>Thành
                                    công</option>
                                <option value="1"
                                    <?php if($this->input->post('select_status') == "1" ){echo "selected";} ?>>Thất bại
                                </option>
                                <option value="30"
                                    <?php if($this->input->post('select_status') == "30" ){echo "selected";} ?>>Đang xử
                                    lý</option>
                                <option value="31"
                                    <?php if($this->input->post('select_status') == "31" ){echo "selected";} ?>>Thẻ đã
                                    nạp trước đó</option>
                                <option value="32"
                                    <?php if($this->input->post('select_status') == "32" ){echo "selected";} ?>>Thẻ bị
                                    khóa</option>
                                <option value="33"
                                    <?php if($this->input->post('select_status') == "33" ){echo "selected";} ?>>Thẻ chưa
                                    kích hoạt</option>
                                <option value="34"
                                    <?php if($this->input->post('select_status') == "34" ){echo "selected";} ?>>Thẻ hết
                                    hạn</option>
                                <option value="35"
                                    <?php if($this->input->post('select_status') == "35" ){echo "selected";} ?>>Sai mã
                                    thẻ</option>
                                <option value="36"
                                    <?php if($this->input->post('select_status') == "36" ){echo "selected";} ?>>Mã
                                    serial không đúng</option>
                                <option value="37"
                                    <?php if($this->input->post('select_status') == "37" ){echo "selected";} ?>>Hết hạn
                                    mức nạp user trong ngày</option>
                                <option value="38"
                                    <?php if($this->input->post('select_status') == "38" ){echo "selected";} ?>>Hết hạn
                                    mức nạp hệ thống trong ngày</option>
                            </select>
                        </td>
                        <td style="">
        ...
        success: function(result) {
            ...
            if (result.transactions == "") {
                ...
                $.each(result.transactions, function(index, value) {
                    result += resultSearchTransction(stt, value.referenceId, value.nickName,
                        value.provider, value.serial, value.pin, value.amount, value
                        .status, value.message, value.code, value.timelog);
                    stt++;
                });
