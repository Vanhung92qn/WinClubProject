...
<?php else : ?>
    <div class="wrapper">
        ...
        <div class="widget">
            ...
            <div class="title">
                <h6>Lịch sử pokego</h6>
            </div>
            <form class="list_filter form" action="<?php echo admin_url('logminigame/logpokego') ?>" method="post">
                <div class="formRow">
                    <table>
                        <tr>
                            <td>
                ...
                <div class="formRow">
                    <table>
                        <tr>
                            ...
                            <td><select id="menhgiavin" name="menhgiavin" style="margin-left: 10px;margin-bottom:-2px;width: 150px;">
                                    <option value="" <?php if ($this->input->post('menhgiavin') == "") {
                                                            echo "selected";
                                                        } ?>>Chọn</option>
                                    <option value="100" <?php if ($this->input->post('menhgiavin') == "100") {
                                                            echo "selected";
                                                        } ?>>100 Win</option>
                                    <option value="1000" <?php if ($this->input->post('menhgiavin') == "1000") {
                                                                echo "selected";
                                                            } ?>>1K Win</option>
                                    <option value="10000" <?php if ($this->input->post('menhgiavin') == "10000") {
                                                                echo "selected";
                                                            } ?>>10K Win</option>
                                </select></td>
                            <td><label id="labelvin" style="margin-left: 80px;margin-bottom:-2px;width: 75px;">Phòng xu:</label></td>

                            <td><select id="menhgiaxu" name="menhgiaxu" style="margin-left: 10px;margin-bottom:-2px;width: 150px;">
                                    <option value="" <?php if ($this->input->post('menhgiaxu') == "") {
                                                            echo "selected";
                                                        } ?>>Chọn</option>
                                    <option value="1000" <?php if ($this->input->post('menhgiaxu') == "1000") {
                                                                echo "selected";
                                                            } ?>>1K Xu</option>
                ...
                <div class="formRow">
                    <table>
                        <tr>
                            <td><label id="labelvin" style="margin-left: 50px;margin-bottom:-2px;width: 70px;">Tiền:</label></td>

                            <td><select id="money_type" name="money_type" style="margin-left: 10px;margin-bottom:-2px;width: 150px;">
                                    <option value="1" <?php if ($this->input->post('money_type') == "1") {
                                                            echo "selected";
                                                        } ?>>Vin</option>
                                    <option value="0" <?php if ($this->input->post('money_type') == "0") {
                                                            echo "selected";
                                                        } ?>>Xu</option>
                                </select></td>
                            <td style="">
                                <input type="submit" id="search_tran" value="Tìm kiếm" class="button blueB" style="margin-left: 40px">
                            </td>
                            <td>
                                <input type="reset" onclick="window.location.href = '<?php echo admin_url('logminigame/logpokego') ?>'; " value="Reset" class="basic" style="margin-left: 20px">
                            </td>
                        </tr>
                    </table>
                </div>
...
<script>
    ...
    function resultlogpokego(stt, referid, nickname, room, linebet, linewin, prizeline, prize, result, money, time_log) {
        var rs = "";
        rs += "<tr>";
        rs += "<td>" + stt + "</td>";
        rs += "<td>" + referid + "</td>";
    ...
    $(document).ready(function() {
        ...
        if ($("#money_type").val() == 1) {
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo admin_url('logminigame/logpokegoajax') ?>",
                data: {
                    phienbc: $("#phienbc").val(),
                    nickname: $("#name").val(),
                ...
                success: function(result) {
                    ...
                    if (result.transactions == "") {
                        ...
                        $.each(result.transactions, function(index, value) {
                            result += resultlogpokego(stt, value.reference_id, value.user_name, value.bet_value, value.lines_betting, value.lines_win, value.prizes_on_line, value.prize, value.result, value.money_type, value.time_log);
                            stt++;
                        });
