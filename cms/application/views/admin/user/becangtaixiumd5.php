...
<?php if ($role === null): ?>
    <section class="content-header">
        <h1>
            Bạn chưa được phân quyền abc <?php echo $role; ?>
        </h1>
    </section>
<?php else: ?>
<div class="wrapper">
    ...
    <div class="container">
            <div>
                <div class="blockTop">
                    <div class="block-md5-ketqua">
                    <div>MD5: <span style="color: green" id="md5hash"></span></div>
                    <!-- <div>Kết quả: <span style="color: red" id="plainText"></span></div> -->
                </div>  
                    ...
                    <div class="row_be_cang">
                        <div id="label-tai" class="column_flex">
                            ...
                            <span id='number_user_tai'style="margin: 0px 5px; color: yellow;">399</span>
                        </div>
                        <div class="column_flex">
                            ...
                            <div style="color: yellow; font-size: 64px; width: 120px; height: 120px" class="icon_tai" title="Thời gian phiên" id="result">
                            </div>
                            </div>
                            <div style="color: yellow ; font-size: x-large;" id="real_time"></div>
                        ...
                        <div id="label-xiu" class="column_flex">
                            ...
                            <div style="color: #ffab00;"><span class="glyphicon glyphicon-user"></span></div>
                            <span id='number_user_xiu' style="margin: 0px 5px; color: yellow;">399</span>
                        </div>
                    </div>
...
<script>
    ...
    $("#save_set_bot").click(function () {
        ...
        if ($("#numberUserTaiMax").val() === "" || $("#numberUserXiuMax").val() === "" || $("#moneyMax").val() === "" || $("#moneyMax").val() === "NaN" || $("#moneyMin").val() === "" || $("#moneyMin").val() === "NaN") {
            alert("Các trường thông tin không được phép để trống ")
        } else if ($("#numberUserTaiMax").val() > 1000 || $("#numberUserXiuMax").val() > 1000) {
            alert("Số bot tối đa là 1000 một bên");
        } else if (moneyMax < moneyMin) {
            alert("Mức cược tối đa phải lớn hơn Mức cược tối thiểu");
        } else {
            updateSetBot(moneyMin, moneyMax, $("#numberUserTaiMax").val(), $("#numberUserXiuMax").val());
            getBotInfo();
        }
    });

    $("#set_bot_Tai").click(function () {
        updateSetBot(moneyMinGlobal, moneyMaxGlobal, numberUserTaiMaxGlobal + 100, numberUserXiuMaxGlobal);
        getBotInfo();
    ...
    $("#set_bot_xiu").click(function () {
        updateSetBot(moneyMinGlobal, moneyMaxGlobal, numberUserTaiMaxGlobal, numberUserXiuMaxGlobal + 100);
        getBotInfo();
        document.getElementById("set_bot_Tai").disabled = true;
        document.getElementById("set_bot_xiu").disabled = true;
    });
    ...
    $("#set_bot_same").click(function () {
        if (numberUserTaiMaxGlobal > numberUserXiuMaxGlobal) {
            console.log(1);
            updateSetBot(moneyMinGlobal, moneyMaxGlobal, numberUserXiuMaxGlobal, numberUserXiuMaxGlobal);
        } else {
            console.log(2);
            updateSetBot(moneyMinGlobal, moneyMaxGlobal, numberUserTaiMaxGlobal, numberUserTaiMaxGlobal);
        }
        getBotInfo();
    ...
    function sendMsg(nickname, message) {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo base_url("admin/user/chatMD5Ajax") ?>",
            data: {
                nickname: nickname,
                content: message,
    ...
    function updateSetBot(moneyMin, moneyMax, numberUserTaiMax, numberUserXiuMax) {
        console.log(moneyMin)
        $.ajax({
            type: "POST",
            url: "<?php echo base_url("admin/user/updateSetBotMD5") ?>",
            data: {
                moneyMin: moneyMin,
                moneyMax: moneyMax,
    ...
    function setStatusBeCang(result) {
        if(result.tai_xiu_be_cang_md5 === 'tai' && $('#autoTai:checked').length == 0) {
            $('#autoTai').trigger('click');
        } else
        if(result.tai_xiu_be_cang_md5 === 'xiu' && $('#autoXiu:checked').length == 0) {
            $('#autoXiu').trigger('click');
        } 
        if(result.tai_xiu_be_cang_md5 === 'auto' && $('#auto:checked').length == 0) {
            $('#auto').trigger('click');
        }
        let kqBeCang = 'Auto';
        if(result.tai_xiu_be_cang_md5?.toLowerCase() == 'tai') {
            kqBeCang = 'Tài';
        }
        if(result.tai_xiu_be_cang_md5?.toLowerCase() == 'xiu') {
            kqBeCang = 'Xỉu';
        }
        document.getElementById("ket_qua_be_cang").innerText = kqBeCang;
        document.getElementById("numberHuTx").innerText = commaSeparateNumber(result.fund_tx_md5_auto_md5);
    }
    ...
    function chatAndListUser(rows) {
        if (rows != null) {
            ...
            const rowXiu = rows.filter((item) => item.cuaDat !== 1);
    ...
    function inforSetBot(rows) {
        ...
        if (rows != null) {
            ...
            html += "<td>" + "~" + (rows.numberUserXiuMax + rows.numberUserTaiMax) + "</td>";
            html += "</tr>";
            html += "</tbody>";
            html += "</table>";
    ...
    function showResultSetTX() {
        $.ajax({
            type: "GET",
            url: "<?php echo base_url("admin/user/getResultBecangTaixiuMD5") ?>",
            success: function (result) {
                obj = JSON.parse(result);
                let data
                if (obj.tai_xiu_be_cang_md5 == 0) {
                    // becangtaixiumd5("auto");
                    alert("Lỗi showResultSetTX không xác định");
                } else if (obj.tai_xiu_be_cang_md5 != null) {
                    setStatusBeCang(obj);
                }
            }
        })
    ...
    function getBotInfo() {
        $.ajax({
            type: "GET",
            url: "<?php echo base_url("admin/user/getInforSetBotMD5") ?>",
            success: function (result) {
                if (result == 0) {
    ...
    (function getListBotChat() {
        $.ajax({
            url: "<?php echo base_url("admin/user/getListBotChatTaixiuMD5Ajax") ?>",
            type: "GET",
            success: function (result) {
                if (result == 0) {
    ...
    function becangtaixiumd5(type) {
        $.ajax({
            type: "POST",
            url: "<?php echo base_url("admin/user/becangtaixiumd5ajax") ?>",
            data: {
                acbe: type,
                nohu: "auto",
            },
    ...
    $("#autoHuTai").click(function () {
        //let isMobileSecure = <?//= $this->session->userdata('isMobileSecure') ?>//;
        //if (isMobileSecure == 1) $("#bsModal3").modal('show');
        becangTaiXiuWNoHu("tai");
        $("#errorvin").html("");
        // }
    });
    ...
    $("#autoHuXiu").click(function () {
        //let isMobileSecure = <?//= $this->session->userdata('isMobileSecure') ?>//;
        //if (isMobileSecure == 1) $("#bsModal4").modal('show');
        becangTaiXiuWNoHu("xiu");
        $("#errorvin").html("");
    ...
    $("#setBotFake").click(function () {
        $.ajax({
            type: "POST",
            url: "<?php echo base_url("admin/user/setBotFakeMD5Ajax") ?>",
            data: {
...
function doconnect(){
  try {
    ws = new WebSocket("<?php echo web_socket() ?>taixiumd5") // ket noi ws server
...
function showMoney(obj) {
    ...
    document.getElementById("md5hash").innerText = obj.taiXiuMd5Hash
    if(obj.taiXiuPlainResult) {
        buildResult(obj);
    }
    ...
    if (obj.bettingRound) {
        ...
        document.getElementById("real_time").innerText = 0;
...
<style>
    ...
    .winning {
        background-image: url("<?php echo public_url('admin/images/md5.gif') ?>");
