...
<div class="wrapper">
    ...
    <div class="widget" style="background: #82d1ff; border-radius: 15px;">
        ...
        <div class="title">
            <h6 style="color: #001f0f;">CẬP NHẬT THÔNG TIN NGÂN HÀNG</h6>
        </div>
...
<script>
    ...
    function listAccFunc(stt, value) {
        ...
        rs += `<td style='color: #01125f;font-weight: bold; text-align: center;'><input id="${value.nickName}_bankName" value="${value.bankName}"/></td>`;
        rs += `<td style='color: #01125f;font-weight: bold; text-align: center;'><input id="${value.nickName}_accountName" value="${value.accountName}"/></td>`;
        rs += `<td style='color: #01125f;font-weight: bold; text-align: center;'><input id="${value.nickName}_bankAccount" value="${value.bankAccount}" /></td>`;
    ...
    function Update(name) {
        ...
        const bankName = $(`#${name}_bankName`).val();
        const bankAccount = $(`#${name}_bankAccount`).val();
        const accountName = $(`#${name}_accountName`).val();
        const id = $(`#${name}_id`).val();
        $.ajax({
            type: "POST",
            url: "<?php echo admin_url('user/updateBankInfoAjax')?>",
            data: {
                nickname: name,
                bankAccount,
                bankName,
	...
	function findAccountByNickname() {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo admin_url('user/findBankByNicknameAjax')?>",
            data: {
                nickname
            },
            ...
            dataType: 'json',
            success: function (result) {
				if(result != null && result.banks?.length) {
					Accfind = listAccFunc(1, result.banks[0]);
