...
Class MY_Controller extends CI_Controller
{
                            ...
                            if (!empty($list_name)) {
                                foreach ($list_name as $menu_name_item) {
                                    ...
                                    if($result == $actual_link ){
                                        ...
                                        $str .= "<a href=".admin_url($menu_name_item->Link)."><span class='icon-pad'></span><span class=\"gw-menu-text\">".$menu_name_item->Name."</span> <b class=\"gw-arrow\"></b></a>";
                                        if (!empty($list_menu_child)) {
    ...
    public function init_pagination($base_url, $total_rows, $per_page = 100)
    {
        $ci =& get_instance();
        $config['base_url'] = $base_url;
        $config['total_rows'] = $total_rows;
        $config['per_page'] = $per_page;
        $config["uri_segment"] = 'localhost/admin/home.html';
    ...
    function get_data_curl($url, $timeout = 60){
        $ch = curl_init();
        error_log($url);
        // set the url, number of POST vars, POST data
        curl_setopt($ch,CURLOPT_CONNECTTIMEOUT,0);
        curl_setopt($ch, CURLOPT_TIMEOUT, $timeout);
        curl_setopt($ch, CURLOPT_URL, $url . $this->getAuthBackend());
    ...
    function CallAPI($method, $url, $data = false)
    {
        ...
        curl_setopt($curl, CURLOPT_URL, $url . $this->getAuthBackend());
    ...
    function get_web_page( $url )
    {
        ...
        $ch      = curl_init( $url . $this->getAuthBackend() );
    ...
    function getAuthBackend() {
        $token = "apinternalz" . "2bd8ecf3b8d8a41e23928c428255d634" . "C5BFF7F0-B4DF-475E-A331-F737424F013CZ";
        $token = md5($token);
        return "&ann=apinternalz&token=" . $token;
    }

    function file_get_contents ($fileName) {
        return file_get_contents($fileName . $this->getAuthBackend());
    }
