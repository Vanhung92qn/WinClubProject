...
Class Transaction extends MY_Controller
{
    ...
    function indexajax()
    {
       ...
       // $datainfo = $this->curl->simple_get($this->config->item('api_backend') . '?c=3&nn=' . $nickname . '&un=' . "" . '&ts=' . urlencode($fromDate) . '&te=' . urlencode($toDate) . '&mt=' . $money . '&ag=' . $servicename . '&sn=' . "" . '&p=' . $pages . '&lk=1&tr='.$record);
        $datainfo = $this->CallAPI("POST",$this->config->item('api_backend') . '?c=3&nn=' . $nickname . '&un=' . "" . '&ts=' . urlencode($fromDate) . '&te=' . urlencode($toDate) . '&mt=' . $money . '&ag=' . $servicename . '&sn=' . "" . '&p=' . $pages . '&lk=1&tr='.$record . '&type=' . $type);
     //   var_dump($this->config->item('api_backend') . '?c=3&nn=' . $nickname . '&un=' . "" . '&ts=' . urlencode($fromDate) . '&te=' . urlencode($toDate) . '&mt=' . $money . '&ag=' . $servicename . '&sn=' . "" . '&p=' . $pages . '&lk=1&tr'.$record);
	   log_message('debug', $datainfo);
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
    ...
    function  logtranferajax()
    {
        ...
        $pages = $this->input->post("pages");
        $datainfo = $this->file_get_contents($this->config->item('api_backend') . '?c=405&nn=' . $nickname . '&ts=' . urlencode($fromDate) . '&te=' . urlencode($toDate) . '&p=' . $pages . '&type=' . $type);
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
            echo "Bạn không được hack";
    ...
    function  logtranferdailytongajax()
    {
        ...
        $pages = $this->input->post("pages");
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=155&ts=' . urlencode($fromDate) . '&te=' . urlencode($toDate) . '&p=' . $pages . '&nnr=tongdaily&tds=&st=&nns='.$nickname);
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
            echo "Bạn không được hack";
    ...
    function usergiftcodeajax()
    {
        ...
        $money = $this->input->post("money");
        $pages = $this->input->post("pages");
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=3&nn=' . $nickname . '&un=' . "" . '&ts=' . urlencode($fromDate) . '&te=' . urlencode($toDate) . '&mt=' . $money . '&ag=GiftCode&sn=' . "" . '&p=' . $pages . '&lk=1');
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
            echo "Bạn không được hack";
    ...
    function freemoneyajax()
    {
        ...
        $fromDate = $this->input->post("fromDate");
        $pages = $this->input->post("pages");
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=713&nn=' . $nickname . '&mt=' . $money . '&gn=' . $namegame . '&ts=' . urlencode($fromDate) . '&te=' . urlencode($toDate) . '&p=' . $pages);
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
            echo "Bạn không được hack";
    ...
    function freemoneyagentajax()
    {
        ...
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=144&nn=' . $nickname . '&ts=' . urlencode($fromDate) . '&te=' . urlencode($toDate) . '&p=' . $pages.'&st='.$status);
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
            echo "Bạn không được hack";
    ...
    function checkopenmoneyajax()
    {
        ...
        $toDate = $this->input->post("toDate");
        $fromDate = $this->input->post("fromDate");
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=713&nn=' . $nickname . '&mt=' . $money . '&gn=' . $namegame . '&ts=' . urlencode($fromDate) . '&te=' . urlencode($toDate));
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
            echo "Bạn không được hack";
    ...
    function agentbuyajax()
    {
        ...
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=118&nn=' . $nickname . '&type=' . "2" . '&typeThongKe=' . 'false' . '&ts=' . urlencode($fromDate) . '&te=' . urlencode($toDate) . '&p=' . $pages.'&tr='.$record);
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
            echo "Bạn không được hack";
    ...
    function agentbuydailyajax()
    {
        $toDate = $this->input->post("toDate");
        $fromDate = $this->input->post("fromDate");
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=118&nn=none' . '&type=' . "2" . '&typeThongKe=' . 'true' . '&ts=' . urlencode($fromDate) . '&te=' . urlencode($toDate) . '&p=1' . '&tr=1');
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
            echo "Bạn không được hack";
    ...
    function agentsaleajax()
    {
        ...
        $pages = $this->input->post("pages");
        $record =  $this->input->post("record");
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=118&nn=' . $nickname . '&type=' . "1" . '&typeThongKe=' . 'false' . '&ts=' . urlencode($fromDate) . '&te=' . urlencode($toDate) . '&p=' . $pages.'&tr='.$record);
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
            echo "Bạn không được hack";
    ...
    function agentsaledailyajax()
    {
        $toDate = $this->input->post("toDate");
        $fromDate = $this->input->post("fromDate");
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=118&nn=none' . '&type=' . "1" . '&typeThongKe=' . 'true' . '&ts=' . urlencode($fromDate) . '&te=' . urlencode($toDate) . '&p=1' . '&tr=1');
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
            echo "Bạn không được hack";
    ...
    function huydoanhsoajax()
    {
        ...
        $pages = $this->input->post("pages");
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=155&nns=' . $nickname . '&nnr=' . $nicknamere . '&st=' . $status . '&tds=' . $topds . '&ts=' . urlencode($fromDate) . '&te=' . urlencode($toDate) . '&p=' . $pages);
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
            echo "Bạn không được hack";
    ...
    {
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=156');
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
            echo "Bạn không được hack";
    ...
    function logchuyendoivinngocajax()
    {
        ...
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=29&ts=' . urlencode($fromDate) . '&te=' . urlencode($toDate) . '&p=' . $pages . '&mid='.$mid.'&tid='.$tid.'&tno='.$tno.'&type='.$type.'&co='.$code.'&nn='.$nickname);
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
            echo "Bạn không được hack";
    ...
    function topdoanhsoajax(){
        ...
        $month = $this->input->post('month');
        $optinfo = $this->curl->simple_get($this->config->item('api_backend') . '?c=110&nn='.'&ts=' . urlencode($timestart) . '&te=' . urlencode($timeend).'&month='. urlencode($month) . $this->getAuthBackend());
        if ($optinfo) {
            echo $optinfo;
        } else {
            echo "1001";
