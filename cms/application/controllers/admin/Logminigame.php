...
Class Logminigame extends MY_Controller
{
    ...
    function indexajax()
    {
        ...
        $datainfo = $this->get_data_curl($this->config->item('api_backend').'?c=137&rid='.$phientx.'&ts='.urlencode($fromDate).'&te='.urlencode($toDate).'&mt='.$money.'&p='.$pages);
        if(isset($datainfo)) {
            echo $datainfo;
        }else{
    ...
    function accounttaixiuajax()
    {
        ...
        $money = $this->input->post("money");
        $pages = $this->input->post("pages");
        $datainfo = $this->get_data_curl($this->config->item('api_backend').'?c=505&rid='.$phientx.'&nn='.$nickname.'&bs='.$cuadat.'&ts='.urlencode($fromDate).'&te='.urlencode($toDate).'&mt='.$money.'&p='.$pages);
        if(isset($datainfo)) {
            echo $datainfo;
        }else{
            echo "Bạn không được hack";
   ...
   function detailphientaixiu(){
    ...
    function detailphientaixiuajax(){
        ...
        $money = $this->input->post("money");
        $pages = $this->input->post("pages");
        $datainfo = $this->get_data_curl($this->config->item('api_backend').'?c=506&nn='.$nickname.'&rid='.$phientx.'&bs='.$cuadat.'&mt='.$money.'&p='.$pages);
        if(isset($datainfo)) {
            echo $datainfo;
        }else{
            echo "Bạn không được hack";
    ...
    function logcaothapajax(){
        ...
        $pages = $this->input->post("pages");
        if($money == 1){
            $datainfo = $this->get_data_curl($this->config->item('api_backend').'?c=503&nn='.$nickname.'&tid='.$phienbc.'&r='.$roomvin.'&ts='.urlencode($fromDate).'&te='.urlencode($toDate).'&mt='.$money.'&p='.$pages);
        }else if($money == 0){
            $datainfo = $this->get_data_curl($this->config->item('api_backend').'?c=503&nn='.$nickname.'&tid='.$phienbc.'&r='.$roomxu.'&ts='.urlencode($fromDate).'&te='.urlencode($toDate).'&mt='.$money.'&p='.$pages);
        }
    ...
    function logminipokerajax(){
        ...
        $pages = $this->input->post("pages");
        if($money == 1){
            $datainfo = $this->get_data_curl($this->config->item('api_backend').'?c=504&nn='.$nickname.'&r='.$roomvin.'&ts='.urlencode($fromDate).'&te='.urlencode($toDate).'&mt='.$money.'&p='.$pages);
        }else if($money == 0){
            $datainfo = $this->get_data_curl($this->config->item('api_backend').'?c=504&nn='.$nickname.'&r='.$roomxu.'&ts='.urlencode($fromDate).'&te='.urlencode($toDate).'&mt='.$money.'&p='.$pages);
        }
    ...
    function logphienbaucuaajax(){
        ...
        $fromDate = $this->input->post("fromDate");
        $pages = $this->input->post("pages");
        $datainfo = $this->get_data_curl($this->config->item('api_backend').'?c=119&rid='.$phienbc.'&r='.$room.'&ts='.urlencode($fromDate).'&te='.urlencode($toDate).'&p='.$pages);
        if(isset($datainfo)) {
            echo $datainfo;
        }else{
            echo "Bạn không được hack";
    ...
    function logbaucuaajax(){
        ...
        $pages = $this->input->post("pages");
        if($money == 1){
            $datainfo = $this->get_data_curl($this->config->item('api_backend').'?c=501&nn='.$nickname.'&rid='.$phienbc.'&r='.$roomvin.'&ts='.urlencode($fromDate).'&te='.urlencode($toDate).'&mt='.$money.'&p='.$pages);
        }else if($money == 0){
            $datainfo = $this->get_data_curl($this->config->item('api_backend').'?c=501&nn='.$nickname.'&rid='.$phienbc.'&r='.$roomxu.'&ts='.urlencode($fromDate).'&te='.urlencode($toDate).'&mt='.$money.'&p='.$pages);
        }
    ...
    function detailbaucuaajax(){
        ...
        $pages = $this->input->post("pages");
        $datainfo = $this->$this->get_data_curl($this->config->item('api_backend').'?c=502&rid='.$phienbc.'&p='.$pages);
        if(isset($datainfo)) {
            echo $datainfo;
        }else{
            echo "Bạn không được hack";
    ...
    function logpokegoajax(){
        ...
        $pages = $this->input->post("pages");
        if($money == 1){
            $datainfo = $this->get_data_curl($this->config->item('api_backend').'?c=121&un='.$nickname.'&rid='.$phienbc.'&bv='.$roomvin.'&ts='.urlencode($fromDate).'&te='.urlencode($toDate).'&mt='.$money.'&p='.$pages);
        }else if($money == 0){
            $datainfo = $this->get_data_curl($this->config->item('api_backend').'?c=121&un='.$nickname.'&rid='.$phienbc.'&bv='.$roomxu.'&ts='.urlencode($fromDate).'&te='.urlencode($toDate).'&mt='.$money.'&p='.$pages);
        }
    ...
    function logkhobauajax(){
        ...
        $fromDate = $this->input->post("fromDate");
        $pages = $this->input->post("pages");
        $datainfo = $this->get_data_curl($this->config->item('api_backend').'?c=122&un='.$nickname.'&rid='.$phienbc.'&bv='.$roomvin.'&ts='.urlencode($fromDate).'&te='.urlencode($toDate).'&p='.$pages.'&gn=KhoBau');
        if(isset($datainfo)) {
            echo $datainfo;
        }else{
            echo "Bạn không được hack";
    ...
    function lognudiepvienajax(){
        ...
        $fromDate = $this->input->post("fromDate");
        $pages = $this->input->post("pages");
        $datainfo = $this->get_data_curl($this->config->item('api_backend').'?c=122&un='.$nickname.'&rid='.$phienbc.'&bv='.$roomvin.'&ts='.urlencode($fromDate).'&te='.urlencode($toDate).'&p='.$pages.'&gn=NuDiepVien');
        if(isset($datainfo)) {
            echo $datainfo;
        }else{
            echo "Bạn không được hack";
    ...
    function logavengerajax(){
        ...
        $fromDate = $this->input->post("fromDate");
        $pages = $this->input->post("pages");
        $datainfo = $this->get_data_curl($this->config->item('api_backend').'?c=122&un='.$nickname.'&rid='.$phienbc.'&bv='.$roomvin.'&ts='.urlencode($fromDate).'&te='.urlencode($toDate).'&p='.$pages.'&gn=SieuAnhHung');
        if(isset($datainfo)) {
            echo $datainfo;
        }else{
            echo "Bạn không được hack";
    ...
    function loggameajax(){
        ...
        $datainfo = $this->get_data_curl($this->config->item('api_backend').'?c=122&un='.$nickname.'&rid='.$phienbc.'&bv='.$roomvin.'&ts='.urlencode($fromDate).'&te='.urlencode($toDate).'&p='.$pages.'&gn='.$gameName);
        if(isset($datainfo)) {
            echo $datainfo;
        }else{
            echo "Bạn không được hack";
    ...
    function logvuongquocvinajax(){
        ...
        $fromDate = $this->input->post("fromDate");
        $pages = $this->input->post("pages");
        $datainfo = $this->get_data_curl($this->config->item('api_backend').'?c=122&un='.$nickname.'&rid='.$phienbc.'&bv='.$roomvin.'&ts='.urlencode($fromDate).'&te='.urlencode($toDate).'&p='.$pages.'&gn=VuongQuocVin');
        if(isset($datainfo)) {
            echo $datainfo;
        }else{
            echo "Bạn không được hack";
