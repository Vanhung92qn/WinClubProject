...
Class Marketing extends MY_Controller
{
    ...
    function listcampaingajax()
    {
        $utm_campaign =  $this->input->post("utm_campaign");
         ...
         $datainfo = $this->file_get_contents($this->config->item('api_backend').'?c=6&utm_campaign='.$utm_campaign.'&utm_medium='.$utm_medium.'&utm_source='.$utm_source.'&ts='.urlencode($fromDate).'&te='.urlencode($toDate));
        if(isset($datainfo)) {
            echo $datainfo;
        }else{
    ...
    function  rechargevndajax(){
        ...
        $toDate = $this->input->post("toDate");
        $fromDate = $this->input->post("fromDate");
        $datainfo = $this->file_get_contents($this->config->item('api_backend').'?c=7&ts='.urlencode($fromDate).'&te='.urlencode($toDate));
        if(isset($datainfo)) {
            echo $datainfo;
        }else{
            echo "Bạn không được hack";
    ...
    function  reportmarketingajax(){
        ...
        $toDate = $this->input->post("toDate");
        $fromDate = $this->input->post("fromDate");
        $datainfo = $this->file_get_contents($this->config->item('api_backend').'?c=142&ts='.urlencode($toDate).'&te='.urlencode($toDate));
        if(isset($datainfo)) {
            echo $datainfo;
        }else{
            echo "Bạn không được hack";
