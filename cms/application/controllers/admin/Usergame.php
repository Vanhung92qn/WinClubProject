...
class Usergame extends MY_Controller
{
    ...
    function indexajax()
    {
        ...
        //print_r(($this->config->item('api_backend') . '?c=104&un=' . $username . '&nn=' . $nickname . '&m=' . $phone . '&fd=' . $fieldname . '&srt=' . $timkiemtheo . '&ts=' . urlencode($fromDate) . '&te=' . urlencode($toDate) . '&dl=' . $typetaikhoan . '&p=' . $pages . '&tr=' . $record . '&bt=' . $taikhoanbot.'&email='.$email.'&lk='.$typetk));
    ...
    function uservinplayajax()
    {
        ...
        $typetk = $this->input->post("typetk");
        $email = $this->input->post("email");
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=104&un=' . $username . '&nn=' . $nickname . '&m=' . $phone . '&fd=' . $fieldname . '&srt=' . $timkiemtheo . '&ts=' . urlencode($fromDate) . '&te=' . urlencode($toDate) . '&dl=' . $typetaikhoan . '&p=' . $pages . '&tr=' . $record . '&bt=0' . '&email=' . $email . '&lk=' . $typetk);
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
            echo "Bạn không được hack";
    ...
    function userbotajax()
    {
        ...
        $pages = $this->input->post("pages");
        $record = $this->input->post("record");
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=104&un=' . $username . '&nn=' . $nickname . '&m=' . $phone . '&fd=' . $fieldname . '&srt=' . $timkiemtheo . '&ts=' . urlencode($fromDate) . '&te=' . urlencode($toDate) . '&dl=' . $typetaikhoan . '&p=' . $pages . '&tr=' . $record . '&bt=1' . '&email=&lk=0');
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
            echo "Bạn không được hack";
    ...
    function lockalluser()
    {
        ...
        if (file_exists(FCPATH . "public/admin/uploads/nicknamelock.csv") != false) {
            ...
            $result = $this->csvreader->parse_file(public_url('admin/uploads/nicknamelock.csv'));
            $listnn = array();
    ...
    function lockalluserajax()
    {
        ...
        $type = $this->input->post("type");
        $txtaction = $this->input->post("txtaction");
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=125&nn=' . $nickname . '&ac=' . $action . '&type=' . $type);
        if (isset($datainfo)) {
            echo $datainfo;
            $data = array(
                'reason' => $lydo,
    ...
    function checkusername()
    {
        $this->data['error'] = "";
        if ($this->input->post("ok")) {
            if (file_exists('public/admin/uploads/checknickname.csv')) {
        ...
        if (file_exists(FCPATH . "public/admin/uploads/checknickname.csv") != false) {
            $this->load->library('CSVReader');
            $result = $this->csvreader->parse_file(public_url('admin/uploads/checknickname.csv'));
            $listnn = array();
            foreach ($result as $row) {
                if (isset($row["Nickname"])) {
                    array_push($listnn, $row["Nickname"]);
    ...
    {
        $this->data['temp'] = 'admin/usergame/resetjackpot';
        $this->load->view('admin/main', $this->data);
    }

    function resetjackpotajax()
    {
        $gamename = urlencode($this->input->post("gamename"));
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=8800&game=' . $gamename);
        echo $datainfo;
    ...
    {
        $gamename = urlencode($this->input->post("gamename"));
        $typejackpot = urlencode($this->input->post("typejackpot"));
        $act = urlencode($this->input->post("act"));
        $nickname = urlencode($this->input->post("nickname"));
        $typeUser = urlencode($this->input->post("typeUser"));
        $moneyPot = '';
        $typeMoney = urlencode($this->input->post("typeMoney"));
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=8799&game=' . $gamename . '&typejackpot=' . $typejackpot .
            '&act=' . $act . '&nickname=' . $nickname . '&typeUser=' . $typeUser . '&betValue=' . $typeMoney . '&totalPrize=' . '');
        echo $datainfo;

    }
    function getTyleHut() {
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=11027');
    ...
    function setTyleHut() {
        ...
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=11026&gameName='.$gamename .'&fee='.$fee);
        $data = json_decode($datainfo);
        if(isset($data -> success)){
            echo $datainfo;
        } else {
    ...
    function congxuajax()
    {
        ...
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=100&nn=' . $nickname . '&mn=' . $money . '&mt=' . "vin" . '&nns=' . $admin_info->FullName . '&rs=' . $reason . '&otp=' . $otp . '&type=' . $typeotp);
        $data = json_decode($datainfo);
        if (isset($data->success)) {
    ...
    function setnohuajax()
    {
        $admin_login = $this->session->userdata('user_id_login');
        $admin_info = $this->admin_model->get_info($admin_login);
        $nickname = urlencode($this->input->post("nickname"));
        dd($nickname);
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=100&nn=' . $nickname . '&mt=' . "vin" . '&nns=' . $admin_info->FullName);
        $data = json_decode($datainfo);
        if (isset($data->success)) {
            if ($data->success == true) {
                if ($data->errorCode == 0) {
    ...
    function logchangemobileajax()
    {
        ...
        $pages = $this->input->post("pages");
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=509&nn=' . $nickname . '&m=' . $mobilenew . '&mo=' . $mobileold . '&p=' . $pages);
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
            echo "Bạn không được hack";
    ...
    function checkinfoajax()
    {
        ...
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=606&nn=' . $nickname . '&ts=' . $todate . '&te=' . $fromdate);
    ...
    function checkmobile()
    {
        ...
        if (file_exists(FCPATH . "public/admin/uploads/checkmobile.csv") != false) {
            $this->load->library('CSVReader');
            $result = $this->csvreader->parse_file(public_url('admin/uploads/checkmobile.csv'));
            $listmb = array();
            foreach ($result as $row) {
                if (isset($row["Sdt"])) {
    ...
    function onlineuser() {
        $this->data['temp'] = 'admin/usergame/onlineuser';
        $this->load->view('admin/main', $this->data);
    }

    function onlineuserajax()
    {
        $nickname = urlencode($this->input->post("nickname"));
        $page = urlencode($this->input->post("pages"));
        $pageSize = urlencode($this->input->post("size"));
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=11011&nn=' . $nickname . '&pageIndex='. $page . '&pageSize='. $pageSize );
    ...
    function detailonlineuserajax()
    {
        $nickname = urlencode($this->input->post("nickname"));
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=11012&nickName=' . $nickname );
