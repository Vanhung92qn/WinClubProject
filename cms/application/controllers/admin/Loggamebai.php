...
//       $this->load->library('mongo_db', array('activate'=>'Loggamebai'),'mongo_db2');
    ...
    function indexajax()
    {
        ...
        $datainfo = $this->get_data_curl($this->config->item('api_backend').'?c=2&nn='.$nickname.'&gn='.$namegame.'&ts=' . urlencode($fromDate) . '&te=' . urlencode($toDate).'&p='.$pages.'&sid='.$sid.'&mt='.$money, 500);
        if(isset($datainfo)) {
            echo $datainfo;
        }else{
    ...
    function detailajax()
    {
       $time = $this->input->post("time");
        ...
        $datainfo = $this->get_data_curl($this->config->item('api_backend').'?c=4&sid='.$sid.'&gn='.$namegame.'&tg=' .$time);
        if(isset($datainfo)) {
            echo $datainfo;
        }else{
            echo "Bạn không được hack";
    ...
    function ticketfreeajax(){
        ...
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=23&nn=' . $nickname . '&id=' . $id . '&cty=' . $source . '&am=' .$gtve . '&ts=' . urlencode($fromDate) . '&te=' . urlencode($toDate) . '&st=' . $status . '&p=' . $pages . '&s=' . $record . '&bot=' . $taikhoan.'&use='.$tinhtrang.'&tid='.$tourid.'&tty='.$typetour);
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
            echo "Bạn không được hack";
    ...
    function reportcodefreeajax(){
       ...
       // var_dump($this->config->item('api_backend') . '?c=25&id=' . $id . '&co=' . $code . '&pkid=' . $loid . '&gn=PokerTour&am='.$amount.'&ct='.$typecode.'&st='.$status.'&nn='.$nickname.'&adi='.$thongtin.'&ts=' . urlencode($fromDate) . '&te=' . urlencode($toDate).'&type='.$search.'&adm='.$accesstoken[1].'&at='.$accesstoken[0]);
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=25&id=' . $id . '&co=' . $code . '&pkid=' . $loid . '&gn=PokerTour&am='.$amount.'&ct='.$typecode.'&st='.$status.'&nn='.$nickname.'&adi='.$thongtin.'&ts=' . urlencode($fromDate) . '&te=' . urlencode($toDate).'&type='.$search.'&adm='.$accesstoken[1].'&at='.$accesstoken[0]);
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
            echo "Bạn không được hack";
    ...
    function reportlocodefreeajax(){
        ...
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=26&id=' . $id .'&gn=PokerTour&am='.$amount.'&ct='.$typecode.'&act='.$nickname.'&ts=' . urlencode($fromDate) . '&te=' . urlencode($toDate));
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
            echo "Bạn không được hack";
    ...
    function  reportcodepokertourajax(){
        ...
        $fromDate = $this->input->post("fromDate");
        $search = $this->input->post("search");
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=27&gn=PokerTour&ts=' . urlencode($fromDate) . '&te=' . urlencode($toDate).'&type='.$search);
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
            echo "Bạn không được hack";
