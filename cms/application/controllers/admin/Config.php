...
Class Config extends MY_Controller
{
    ...
    function getdata()
    {
        ...
        $txtstatusxizach = intval($this->input->post("txtstatusxizach"));
        $txtipxizach = $this->input->post("txtipxizach");
        $txtportxizach = intval($this->input->post("txtportxizach"));
        ...
        $txtstatuscoup = intval($this->input->post("txtstatuscoup"));
        $txtipcoup = $this->input->post("txtipcoup");
        $txtportcoup = intval($this->input->post("txtportcoup"));
        $txtstatuscotuong = intval($this->input->post("txtstatuscotuong"));
        $txtipcotuong = $this->input->post("txtipcotuong");
        $txtportcotuong = intval($this->input->post("txtportcotuong"));
        $txtstatuscaro = intval($this->input->post("txtstatuscaro"));
        $txtipcaro = $this->input->post("txtipcaro");
        $txtportcaro = intval($this->input->post("txtportcaro"));
        ...
        if ($configpf == "wp" || $configpf == "ios" || $configpf == "web" || $configpf == "ad") {
            $list = array(
                ...
                "coup" => array(
                    "status" => $txtstatuscoup,
                    "ip" => $txtipcoup,
                    "port" => $txtportcoup
                ),
                "cotuong" => array(
                    "status" => $txtstatuscotuong,
                    "ip" => $txtipcotuong,
                    "port" => $txtportcotuong
                ),
                "caro" => array(
                    "status" => $txtstatuscaro,
                    "ip" => $txtipcaro,
                    "port" => $txtportcaro
                ),
                "poker" => array(
    ...
    {
        $datainfo = $this->file_get_contents($this->config->item('api_backend') . '?c=601&pf=' . "" . '&nm=' . "");
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
    ...
    function editajax()
    {
        ...
        $datainfo = $this->file_get_contents($this->config->item('api_backend') . '?c=601&pf=' . $configpf . '&nm=' . $nmconfig);
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
            echo "Bạn không được hack";
    ...
    function addconfigajax(){
        $value = urlencode($this->input->post('data'));
        $name = $this->input->post('name_config');
        $version = $this->input->post('version');
        $platform = $this->input->post('platform');
        $dataInfo =  $this->file_get_contents($this->config->item('api_backend') . '?c=602&gv=' . $value . '&gn=' . $name . '&v='. $version . '&pf='. $platform);
        if (isset($dataInfo)) {
            echo $dataInfo;
        } else {
            echo "Bạn không được hack";
    ...
    function successeditajax()
    {
        ...
        $datainfo = $this->file_get_contents($this->config->item('api_backend') . '?c=603&gid=' . $idconfig . '&gv=' . $valueconfig . '&v=' . $versionconfig . '&pf=' . $configpf);
        if (isset($datainfo)) {
            $data = array(
    ...
    function configajax()
    {
        $datainfo = $this->file_get_contents($this->config->item('api_url') . '?c=7');
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
            echo "Bạn không được hack";
    ...
    {
        $this->data['temp'] = 'admin/config/managesocial';
        $this->load->view('admin/main', $this->data);
    }
    function managesocialajax() 
    {
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=11016');
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
            echo "Bạn không được hack";
    ...
    function updatelinkajax()
    {
        ...
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=11017&' . $key. '=' . $value);
