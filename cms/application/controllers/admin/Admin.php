...
Class Admin extends MY_Controller
{
    ...
    function addadminajax(){
        ...
        $datainfo = $this->file_get_contents($this->config->item('api_backend').'?c=103&nn='.$nickname.'&st=100');
        if(isset($datainfo)) {
            echo $datainfo;
        }else{
    ...
    function getinfoajax(){
       ...
       // var_dump(base64_encode($accesstoken.'|'.$admin_info->FullName.'|'.'fU3z7wP0IeFOPntKXcRifUDTGbV8AXyI'));
        ...
        $datainfo = $this->file_get_contents($this->config->item('api_backend').'?c=102&nn='.$nickname,false, $context);
        if(isset($datainfo)) {
    ...
    function delete()
    {
        ...
        if($admin_info->ID == $id) {
            ...
            $where = array("User_ID"=>$id);
            $this->userrole_model->del_rule($where);
             $this->file_get_contents($this->config->item('api_backend').'?c=103&nn='.$nickname.'&st=0');
