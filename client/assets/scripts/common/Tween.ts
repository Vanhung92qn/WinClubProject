...
namespace common {
    ...
    export class Tween extends cc.Component {
        ...
        /**
         * 
         * @param label => là node trên UI ví dụ lblTotalTai
         * @param toNumber 
         * @param duration 
         * @param format 
         */
        static numberTo(label: cc.Label, toNumber: number, duration: number, format: (n: number) => string = (n) => { return Utils.formatNumber(n) }) {
            ...
            listener.callback = (p: number) => {
                //update giá trị cho label khi có kết quả trả về 
                label.string = format(parseInt("" + (startNumber + distance * p)));
            }
        }

        static randomHu(jackpotLabel: number): number {
            let randomNumber = Utils.randomRangeInt(152646626,912565949);
            if (jackpotLabel > randomNumber) {
                return jackpotLabel - randomNumber;
            } else {
                return randomNumber;
            }
        }
