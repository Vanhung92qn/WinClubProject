...
export default class TaiXiuKuBetNetWorkClient extends NetworkClient {
    private static instance: TaiXiuKuBetNetWorkClient;
    ...
    public static getInstance(): TaiXiuKuBetNetWorkClient {
        if (this.instance == null) {
            this.instance = new TaiXiuKuBetNetWorkClient();
        }
    ...
    protected onError(ev: Event) {
     ...
     connect() {
        super.connect(Configs.App.HOST_TAIXIU_LIVE_KUBET.host, Configs.App.HOST_TAIXIU_LIVE_KUBET.port);
    }

    protected onOpen(ev: Event) {
        super.onOpen(ev);
        this.send(new cmd.SendLogin(Configs.Login.Nickname, Configs.Login.AccessToken));
        console.log("tx kub connected");
