...
export default class TaiXiuMD5NetWorkClient extends NetworkClient {
    private static instance: TaiXiuMD5NetWorkClient;
    ...
    public static getInstance(): TaiXiuMD5NetWorkClient {
        if (this.instance == null) {
            this.instance = new TaiXiuMD5NetWorkClient();
        }
    ...
    public checkConnect(onLogined: () => void = null) {
        ...
        if (!this.isConnected()) {
            // App.instance.showErrLoading("Đang đăng nhập taixiumd5");
            this.connect();
            return;
        }
    ...
    protected onError(ev: Event) {
     ...
     connect() {
        super.connect(Configs.App.HOST_TAIXIU_MD5.host, Configs.App.HOST_TAIXIU_MD5.port);
    }
