...
export default class LoadingController extends cc.Component {
    ...
    loadBundleLobby() {
        ...
        return BundleControl.loadBundle('Lobby').then((bundle) => {
            ...
            if (typeof App !== 'undefined' && App.instance) {
                App.instance.lobbyBundle = bundle;
            }
            return new Promise((resolve, reject) => {
                // OPTIMIZATION: Load only the Lobby scene instead of ALL assets.
                // Before: bundle.loadDir('') loaded ~40MB (textures, spine, audio, 25 prefabs)
                // After: bundle.loadScene('Lobby') loads only scene + direct deps (~5-7MB)
                // Popup prefabs are now lazy-loaded on-demand via PopupManager.
