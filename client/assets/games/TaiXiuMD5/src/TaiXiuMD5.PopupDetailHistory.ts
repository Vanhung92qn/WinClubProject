...
export default class PopupDetailHistory extends Dialog {
    ...
    private loadData() {
        ...
        this.lblSession.string = "" + this.session;
        this.lblTime.string = "";
        Http.get(Configs.App.API, { "c": 55102, "rid": this.session, "mt": Configs.App.MONEY_TYPE }, (err, res) => {
            if (err != null) return;
            this.historiesTai = [];
            this.historiesXiu = [];
            if (res.success) {
                if(res["resultTX"] !== null) {
                    ...
                    //this.lblTotalRefundTai.string = this.numFormatter(res["resultTX"]["totalRefundTai"]) + "";
                    //this.lblTotalRefundXiu.string = this.numFormatter(res["resultTX"]["totalRefundXiu"]) + "";
                    ...
                    this._md5String = res["resultTX"]["md5TextResult"];
                    this._plainText = res["resultTX"]["plantTextResult"];
                    this.md5String.string = this._md5String;
                    this.plainText.string = this._plainText;
    ...
    public actNextSession() {
        this.session++;
        if (this.session > TaiXiuMD5Controller.instance.histories[TaiXiuMD5Controller.instance.histories.length - 1].session) {
            this.session = TaiXiuMD5Controller.instance.histories[TaiXiuMD5Controller.instance.histories.length - 1].session;
            return;
        }
        this.loadData();
    }
    ...
    private onCopyResultString(event, data) {
        let msgCopy = ["Sao chép chuỗi MD5 thành công", "Sao chép chuỗi kết quả thành công"];
        if(data === "md5") {
            Utils.copyTextToClipboard(this._md5String);
            App.instance.actShowThongBao(msgCopy[0]);
        } else {
            Utils.copyTextToClipboard(this._plainText);
            App.instance.actShowThongBao(msgCopy[1]);
        }
    }
}
