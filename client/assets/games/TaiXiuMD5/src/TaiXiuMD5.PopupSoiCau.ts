...
namespace taixiumini {
    ...
    export class PopupSoiCau extends Dialog {
        ...
        private drawPage1() {
            let data = TaiXiuMD5Controller.instance.histories.slice();
            if (data.length > 21) {
        ...
        private drawPage2() {
            ...
            var data = [];
            var curData = [];
            var count = TaiXiuMD5Controller.instance.histories.length;
            var countTai = 0;
            var countXiu = 0;
            if (count > 1) {
                var dices = TaiXiuMD5Controller.instance.histories[0].dices;
                var score = dices[0] + dices[1] + dices[2];
                var isTai = score >= 11;
                var maxItem = 5;
                for (var i = 0; i < count; i++) {
                    dices = TaiXiuMD5Controller.instance.histories[i].dices;
                    score = dices[0] + dices[1] + dices[2];
                    var _isTai = score >= 11;
            ...
            let countTai2 = 0;
            let countXiu2 = 0;
            data = TaiXiuMD5Controller.instance.histories.slice();
            if (data.length > 100) {
                data.splice(0, data.length - 100);
            }
            for (let i = 0; i < data.length; i++) {
