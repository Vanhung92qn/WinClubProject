...
export default class BaCayController extends cc.Component {
    ...
    @property(cc.Node)
    notifyTimeNothing: cc.Node = null;
    ...
    ping() {
        ...
        this.timeout += 5;
        BaCayNetworkClient.getInstance().send(new cmd.CmdSendPing());
    }
    ...
    setupMatch(data: cmd.ReceivedJoinRoomSucceed) {
        ...
        switch (this.gameState) {
            case 0: // waiting
                this.startNothingActionCountDown(countDownTime);
                break;
    ...
    processBetting(rate) {
        ...
        cc.log("BaCay processBetting fillRange : ", this.actionBetting.children[0].getComponent(cc.Sprite).fillRange);
        this.actionBetting.children[0].getComponent(cc.Sprite).fillRange = rate;
    }

    startNothingActionCountDown(turnTime) {
        cc.log("BaCay startNothingCountDown turnTime : ", turnTime);
        this.timeDoNothing = turnTime;
        this.notifyTimeNothing.active = true;
        this.processNothing(1);
        this.unschedule(this.intervalNothing);
        this.schedule(this.intervalNothing = () => {
            this.timeDoNothing--;
            var rate = (this.timeDoNothing / turnTime).toFixed(1);
            this.processNothing(rate);
            if (this.timeDoNothing < 1) {
                this.unschedule(this.intervalNothing);
                this.notifyTimeNothing.active = false;
            }
        }, 1);
    }

    processNothing(rate) {
        cc.log("BaCay processDoNothing rate : ", rate);
        cc.log("BaCay processDoNothing fillRange : ", this.notifyTimeNothing.children[0].getComponent(cc.Sprite).fillRange);
        this.notifyTimeNothing.children[0].getComponent(cc.Sprite).fillRange = rate;
    }
        ...
        BaCayNetworkClient.getInstance().addListener((data) => {
            ...
            switch (inpacket.getCmdId()) {
                case cmd.Code.LOGIN:
                    ...
                    cc.log("3C Login res:", inpacket.getCmdId());
                    this.refeshListRoom();
                ...
                case cmd.Code.GET_LIST_ROOM:
                    {
                        ...
                        cc.log(res);
                        let listRoom = res.list.sort((room1, room2) => {
                            return room1.moneyBet - room2.moneyBet;
                        })
                ...
                case cmd.Code.JOIN_ROOM_SUCCESS:
                    {
                        ...
                        cc.log("BaCay JoinRoomSuccess res: ", JSON.stringify(res));
                        this.closeUIRoom();
                        this.setupMatch(res);
                    }
                    break;
    ...
    stopAllTime() {
        this.notifyTimeStart.active = false;
        this.notifyTimeBet.active = false;
        this.notifyTimeEnd.active = false;
    }
