...
export default class PopupHistory extends Dialog {
    ...
    private loadData() {
        Http.get(Configs.App.API, { "c": 137, "p": this.page, "un": Configs.Login.Nickname, "gn": GameName.SEXY_DANCE}, (err, res) => {
            if (err != null) return;
            ...
            for (let i = 0; i < this.items.length; i++) {
                ...
                if (i < res["results"].length) {
                    ...
                    clickEventHandler.component = "SlotSexyDance.PopupHistory";
    ...
    actShowDetail(event, data) {
        ...
        let symbolPrefix = `symbol_${SlotSexyDanceSlotSexyDanceController.getInstance().betIdx + 1}_`;
        ...
        if(this._currentSelectedHistory.rs !== 7) {
            switch(id) {
                ...
                case 1:
                    sprite.active = false;
                    nodeSkeleton.active = true;
                    skeleton.skeletonData = this.listAnimFB[1];
                    skeleton.setAnimation(0, 'animation', true);
                    break;
                case 3:
                    sprite.active = false;
                    nodeSkeleton.active = true;
                    skeleton.skeletonData = this.listAnimFB[2];
                    skeleton.setAnimation(0, 'animation', true);
                    break;
                default:
                    sprite.active = true;
