...
export default class PopupDetailHistory extends Dialog {
    ...
    public actNextSession() {
        this.session++;
        if (this.session > TaiXiuLiveKubController.instance.histories[TaiXiuLiveKubController.instance.histories.length - 1].session) {
            this.session = TaiXiuLiveKubController.instance.histories[TaiXiuLiveKubController.instance.histories.length - 1].session;
            return;
        }
        this.loadData();
    }
