...
export class PopupBonus extends cc.Component {
    ...
    start() {
        for (let i = 0; i < this.listItemBonus.length; i++) {
            ...
            node.on("click", () => {
                SlotFAFSlotFAFController.getInstance().playSoundClickBonusItem();
                ...
                if(this.left == 0) {
                    SlotFAFSlotFAFController.getInstance().playSoundOver();
                    ...
                    node.runAction(
                        cc.sequence(
                            ...
                            cc.callFunc(() => {
                                ...
                                SlotFAFSlotFAFController.getInstance().playSoundCar();
                            })
                        )
                    );
        ...
        for (let i = 0; i < this.outDoorItemsNode.length; i++) {
            ...
            node.on(cc.Node.EventType.TOUCH_END, () => {
                node.off(cc.Node.EventType.TOUCH_END);
                SlotFAFSlotFAFController.getInstance().playSoundClickTreasure();
                node.color = cc.Color.WHITE;
                node.getComponent(sp.Skeleton).setAnimation(0, i + 1 + "_mo_" + (this.factors[0]), false);
                ...
                nodeRate.runAction(
                    cc.sequence(
                        ...
                        cc.spawn(
                            cc.moveTo(.5, cc.v2(nodeRate.x, nodeRate.y + 230)),
                        ...
                        cc.callFunc(() => {
                            ...
                            for(let j = 0; j < this.outDoorItemsNode.length; j++) {
                                if(i !== j) {
                                    ...
                                    otherItemNode.getComponent(sp.Skeleton).setAnimation(0, j + 1 + "_mo_1", false);
                                    ...
                                    otherRateNode.runAction(
                                        cc.sequence(
                                            cc.delayTime(.5),
                                            cc.spawn(
                                                cc.moveTo(.5, cc.v2(otherRateNode.x, otherRateNode.y + 230)),
                                                cc.scaleTo(.3, 1).easing(cc.easeBackOut())
                                            ),
                                        )
                                    )
    ...
    resetAll() {
        for (var t = 0; t < this.outDoorItemsNode.length; t++) {
            if(this.outDoorNode.active) {
                this.outDoorItemsNode[t].getComponent(sp.Skeleton).setAnimation(0, t + 1 +"_dong", true);
        ...
        for(let i = 0 ; i < this.listItemBonus.length; i++) {
            let node = this.listItemBonus[i];
            let nodeBG = this.nodeItemBG.children[i];
            node.position = nodeBG.position;
            node.scale = 1;
            node.angle = 0;
        }
