...
export class PopupJackpotHistory extends Dialog {
    ...
    private loadData() {
        Http.get(Configs.App.API, {"c": 138, "p": this.page, "gn": GameName.FAST_AND_FURIOUS}, (err, res) => {
            if (err != null) return;
            ...
            for (let i = 0; i < this.items.length; i++) {
                ...
                if (i < res["results"].length) {
                    ...
                    clickEventHandler.component = "SlotFAF.PopupJackpotHistory";
    ...
    actShowDetail(event, data) {
        ...
        for(let i = 0 ; i < this.columnsWild.length; i++) {
            this.columnWild.children[i + 1].active = true;
        }
