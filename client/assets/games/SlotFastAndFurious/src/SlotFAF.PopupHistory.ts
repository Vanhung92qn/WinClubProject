...
export default class PopupHistory extends Dialog {
    ...
    private loadData() {
        Http.get(Configs.App.API, { "c": 137, "p": this.page, "un": Configs.Login.Nickname, "gn": GameName.FAST_AND_FURIOUS}, (err, res) => {
            if (err != null) return;
            ...
            for (let i = 0; i < this.items.length; i++) {
                ...
                if (i < res["results"].length) {
                    ...
                    clickEventHandler.component = "SlotFAF.PopupHistory";
    ...
    actShowDetail(event, data) {
        ...
        if(this._currentSelectedHistory.rs !== 7) {
            ...
            for(let i = 0 ; i < this.columnsWild.length; i++) {
                this.columnWild.children[i].active = true;
            }
        }
    ...
    setupIconById(nodeItem, id) {
        ...
        let nodeSkeletonSpecial = nodeItem.getChildByName("skeletonSpecial");
        let nodeSkeleton = nodeItem.getChildByName("skeleton");
        let skeletonSpecial = nodeSkeletonSpecial.getComponent(sp.Skeleton);
        let skeleton = nodeSkeleton.getComponent(sp.Skeleton);
        sprite.active = false;
        if(this._currentSelectedHistory.rs !== 7) {
            if(this._itemPrefix.localeCompare("3_") === 0) {
                switch(id) {
                    case 0:
                        sprite.active = false;
                        nodeSkeletonSpecial.active = true;
                        nodeSkeleton.active = false;
                        skeletonSpecial.setAnimation(0, "Free-Items", true);
                        break;
                    case 1:
                        sprite.active = false;
                        nodeSkeletonSpecial.active = true;
                        nodeSkeleton.active = false;
                        skeletonSpecial.setAnimation(0, "Bonus-Items", true);
                        break;
                    case 2:
                        sprite.active = true;
                        nodeSkeletonSpecial.active = false;
                        nodeSkeleton.active = false;
                        sprite.getComponent(cc.Sprite).spriteFrame = this.spriteAtlasIcon.getSpriteFrame("symbol_3_0");
                        break;
                    case 3:
                        sprite.active = false;
                        nodeSkeletonSpecial.active = true;
                        nodeSkeleton.active = false;
                        skeletonSpecial.setAnimation(0, "jackpot-Items", true);
                        break;
                    case 4:
                        sprite.active = false;
                        nodeSkeletonSpecial.active = false;
                        nodeSkeleton.active = true;
                        skeletonSpecial.setAnimation(0, "3_8", true);
                        break;
                    case 5:
                        sprite.active = false;
                        nodeSkeletonSpecial.active = false;
                        nodeSkeleton.active = true;
                        skeleton.setAnimation(0, "3_1", true);
                        break;
                    case 6:
                        sprite.active = false;
                        nodeSkeletonSpecial.active = false;
                        nodeSkeleton.active = true;
                        skeleton.setAnimation(0, "3_5", true);
                        break;
                    case 7:
                        sprite.active = false;
                        nodeSkeletonSpecial.active = false;
                        nodeSkeleton.active = true;
                        skeleton.setAnimation(0, "3_6", true);
                        break;
                    case 8:
                        sprite.active = false;
                        nodeSkeletonSpecial.active = false;
                        nodeSkeleton.active = true;
                        skeleton.setAnimation(0, "3_3", true);
                        break;
                    case 9:
                        sprite.active = false;
                        nodeSkeletonSpecial.active = false;
                        nodeSkeleton.active = true;
                        skeleton.setAnimation(0, "3_7", true);
                        break;
                    case 10:
                        sprite.active = false;
                        nodeSkeletonSpecial.active = false;
                        nodeSkeleton.active = true;
                        skeleton.setAnimation(0, "3_4", true);
                        break;
                }
            } else {
                sprite.active = true;
    ...
    runAnimationSpine(nodeItem) {
        ...
        let nodeSkeletonLow = nodeItem.getChildByName("skeletonSpecial");
    ...
    private stopShowLinesWin() {
        ...
        for (var i = 0; i < this.columnWild.childrenCount; i++) {
            this.columnWild.children[i].scale = 0;
        }
