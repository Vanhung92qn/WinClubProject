...
import cmd from "./SlotHalloween.Cmd";
import Tween from "../../../scripts/common/Tween";
import BroadcastReceiver from "../../../scripts/common/BroadcastReceiver";
import App from "../../../scripts/common/App";
import Configs from "../../../scripts/common/Configs";
import PopupBonus from "./SlotHalloween.PopupBonus";
import TrialResults from "./SlotHalloween.TrialResults";
...
import SlotHalloweenItemsController from "./SlotHalloweenItemsController";
import RollerController from "../../../scripts/common/slot/RollerController";

const { ccclass, property } = cc._decorator;

@ccclass
export default class SlotHalloweenSlotHalloweenController extends cc.Component {
    ...
    effectFreeSpin = null;
    @property(cc.Node)
    effectFreeSpinAnim = null;
    @property(cc.Prefab)
    popupBonus: cc.Prefab = null;
    @property(cc.Prefab)
    popupGuide: cc.Prefab = null;
    @property(cc.Prefab)
    popupSetting: cc.Prefab = null;
    ...
    @property({ type: cc.AudioClip })
    soundShowLineWin = null;
    @property({ type: cc.AudioClip })
    soundFreeSpin = null;
    @property({ type: cc.AudioClip })
    soundWin = null;
    @property({ type: cc.AudioClip })
    soundJackPotXuRoi = null;
    ...
    public static _instance: SlotHalloweenSlotHalloweenController = null;

    protected onLoad() {
        cc.audioEngine.stopAll();
        this.rollerCtrl = this.rollerNode.getComponent(RollerController);
        let self = this;
        this.animNguoiBi.setCompleteListener((anim) => {
            if(anim.animation.name === "ChucMung") {
                self.animNguoiBi.setAnimation(0, "idle", true);
                self.animNguoiBi.timeScale = 1;
            }
        })
        ...
        App.instance.setButtonMiniGamesPosition(this.nodeMiniGamesPosition.position);
    }
    ...
    start() {
        ...
        if(SlotHalloweenSlotHalloweenController._instance === null) {
            SlotHalloweenSlotHalloweenController._instance = this;
        }
                ...
                case cmd.Code.GAME_INFO:
                    {
                        ...
                        freeSpinNode.active = this._isInFreeSpin;
                    }
                    break;
        ...
        console.log("SlotHalloweenController started");
        ...
        this.spinNode.on(cc.Node.EventType.TOUCH_START, touch => {
            if(self.spinAuto) {
                self.spinAuto = false;
                self.animSpinNode.setAnimation(0, "Spin", true);
            }
                        ...
                        cc.sequence(
                            ...
                            cc.callFunc(function() {
                                if(self.autoProgressBar.progress === 0) {
                                    self.animSpinNode.setAnimation(0, "Spin_Hold", true);
                                }
                                self.autoProgressBar.progress += 1/30;
                                if(self.autoProgressBar.progress >= 1) {
                                    self.animSpinNode.setAnimation(0, "Spin_Stop", true);
        ...
        if (this.lastSpinRes.prize > 0) {
            ...
            for (let i = 0; i < linesWin.length; i++) {
                ...
                actions.push(cc.callFunc(() => {
                    line.active = true;
                    this.playSoundEffect(this.soundShowLineWin);
                ...
                if(this.spinAuto) {
                    ...
                    actions.push(cc.callFunc(() => {
                        ...
                        for (let j = 0; j < 5; j++) {
                            ...
                            rolls[j].children.forEach(child => child.opacity = 100);
                            rolls[j].children[2 - itemRow].opacity = 255;
                            rolls[j].children[2 - itemRow].stopAllActions();
                            rolls[j].children[2 - itemRow].getComponent(SlotHalloweenItemsController).showAnimItem(1.5, true, false);
                        }
                    }));
                    actions.push(cc.delayTime(1.5));
                    actions.push(cc.callFunc(() => {
    ...
    actComeBackLobby() {
        ...
        cc.audioEngine.stopAll();
        SlotHalloweenSlotHalloweenController._instance = null;
        App.instance.loadSceneFromBundle("Lobby", {"src": "Lobby"});
    }
    ...
    actSetting() {
        ...
        let popupSettingContainer = popupSetting.getChildByName('Container');
        popupSettingContainer.scale = 0;
        popupSettingContainer.runAction(
            cc.sequence(
                cc.scaleTo(.25, 1.1, 1.1),
                cc.scaleTo(.1, 1, 1),
            )
        )
        this.nodeMain.addChild(popupSetting);
    }
    ...
    onRollDone() {
        ...
        if(this.toggleBoost.isChecked) {
            MusicPlayer.getInstance().playEffect("Sounds/halloween/halloween_rollstop");
        }
        if(!this.spinAuto) {
        ...
        if(!this.isRollingTimeOut) {
            ...
            if(this.moneyExchange > 0) {
                if(this.animNguoiBi.animation !== "ChucMung") {
                    this.animNguoiBi.setAnimation(0, "ChucMung", false);
                }
                this.playSoundEffect(this.soundWin);
            }
        }
        if(!this.spinAuto) {
    ...
    btnRollClick(t) {
        if(this.rollerCtrl.rolling) {
            ...
            this.animSpinNode.setAnimation(0, "Spin_Stop", true);
            if (!this.isPlayingTrial) {
    ...
    checkBigWin() {
        let bigWinAnimNode = this.effectBigWin.children[1];
        if(this.hasBigWin) {
            this.effectBigWin.stopAllActions();
            this.effectBigWin.active = true;
            this.lblBigWin.node.scale = 0;
            this.lblBigWin.string = "";
            bigWinAnimNode.getComponent(sp.Skeleton).setAnimation(0, "animation", false);
            bigWinAnimNode.getComponent(sp.Skeleton).timeScale = 0.8;
            cc.audioEngine.play(this.soundBigWin, false, 1);
            this.effectBigWin.runAction(
                cc.sequence(
                    cc.fadeIn(.8),
                    cc.callFunc(() => {
                        this.lblBigWin.node.stopAllActions();
                        GameUtil.runAnimationMoneyWithColom(this.lblBigWin, 0, this.moneyExchange, 2);
                        this.lblBigWin.node.runAction(cc.scaleTo(.7, 1))
                    }),
                    cc.delayTime(3),
                    cc.callFunc(() => {
                        bigWinAnimNode.stopAllActions();
                        bigWinAnimNode.runAction(cc.fadeOut(.98))
                    }),
                    cc.delayTime(1),
                    cc.callFunc(() => {
                        bigWinAnimNode.stopAllActions();
                        bigWinAnimNode.opacity = 255;
                        this.hasBigWin = false;
                        this.checkAutoSpin();
                        this.effectBigWin.active = false;
                    })
                ...
                this.effectFreeSpin.runAction(
                    cc.sequence(
                        ...
                        cc.callFunc(() => {
                            ...
                            this.effectFreeSpin.opacity = 255;
                            this.effectFreeSpinAnim.getComponent(sp.Skeleton).setAnimation(0, "FreeSpin", false);
                            this.effectFreeSpin.stopAllActions();
                            this.effectFreeSpin.runAction(
                                cc.sequence(
                                    cc.delayTime(1.5),
                                    cc.callFunc(() => {
                                        this.effectFreeSpinAnim.stopAllActions();
                                        this.effectFreeSpinAnim.runAction(cc.fadeOut(.98))
                                    }),
                                    cc.delayTime(1),
                                    cc.callFunc(() => {
                                        this.effectFreeSpinAnim.stopAllActions();
                                        this.effectFreeSpinAnim.opacity = 255;
    ...
    checkJackPot() {
        ...
        if(this.hasJackPot) {
            this.lblJackPotWin.node.scale = 0;
            this.playSFXNoHu();
            this.effectJackpot.active = true;
            this.effectJackpot.stopAllActions();
            jackPotAnimation.opacity = 255;
            jackPotAnimation.getComponent(sp.Skeleton).setAnimation(0, "Jackpot_opening", false);
            this.effectJackpot.runAction(
                cc.sequence(
                    ...
                    cc.delayTime(2.5),
                    cc.callFunc(() => {
                        this.lblJackPotWin.node.runAction(cc.scaleTo(.5 , 1));
                        jackPotAnimation.getComponent(sp.Skeleton).setAnimation(0, "Jackpot_loop", true);
                        this.playSFXXuRoi();
                        GameUtil.runAnimationMoneyWithColom(this.lblJackPotWin, 0, this.moneyExchange, 1.5);
                    }),
                    cc.delayTime(5),
                    cc.callFunc(() => {
                        this.hasJackPot = false;
    ...
    playSFXRoll() {
        // this.playSoundEffect(this.soundSpinStart, true);
        this.playSoundEffectLoop("Sounds/halloween/halloween_roll");
    }
    ...
    playSFXNoHu() {
        this.playSoundEffect(this.soundJackpot);
        // MusicPlayer.getInstance().playEffect("Sounds/cowboy/cowboy_jackpot");
    }

    public static getInstance(): SlotHalloweenSlotHalloweenController {
    ...
    playSoundEffect(soundEffect, isLoop = false) {
        if(GameConfigManager.getInstance().enableSound) {
            cc.audioEngine.playEffect(soundEffect, isLoop);
        }
    }

    playSFXXuRoi() {
        this.playSoundEffect(this.soundJackPotXuRoi);
    }
