...
export default class PopupHistory extends Dialog {
    ...
    private loadData() {
        Http.get(Configs.App.API, { "c": 137, "p": this.page, "un": Configs.Login.Nickname, "gn": GameName.BONG_LAI_CAC}, (err, res) => {
            if (err != null) return;
            ...
            for (let i = 0; i < this.items.length; i++) {
                ...
                if (i < res["results"].length) {
                    ...
                    clickEventHandler.component = "SlotBLC.PopupHistory";
    ...
    actShowDetail(event, data) {
        ...
        let symbolPrefix = `symbol_${SlotBLCSlotBLCController.getInstance().betIdx + 1}_`;
        ...
        if(this._currentSelectedHistory.rs !== 7) {
            ...
            for(let i = 0 ; i < this.columnsWild.length; i++) {
                console.log(this.columnsWild[i]);
