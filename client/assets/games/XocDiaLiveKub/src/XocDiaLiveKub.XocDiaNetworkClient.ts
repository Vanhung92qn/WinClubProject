...
export default class XocDiaLiveKubNetworkClient extends NetworkClient {
    private static instance: XocDiaLiveKubNetworkClient;

    private listeners: Array<NetworkListener> = new Array<NetworkListener>();

    public static getInstance(): XocDiaLiveKubNetworkClient {
        if (this.instance == null) {
            this.instance = new XocDiaLiveKubNetworkClient();
        }
    ...
    public connect() {
        super.connect(Configs.App.HOST_XOCDIA_LIVE_KUBET.host, Configs.App.HOST_XOCDIA_LIVE_KUBET.port);
    }
