...
import MauBinhNetworkClient from "../../../scripts/networks/MauBinh.NetworkClient";
...
export default class MauBinhController extends cc.Component {
    ...
    @property({ type: cc.AudioClip })
    soundTiengDong: cc.AudioClip = null;
    @property({ type: cc.AudioClip })
    soundChiaBai: cc.AudioClip = null;
    @property({ type: cc.AudioClip })
    soundThoatGame: cc.AudioClip = null;
    @property({ type: cc.AudioClip })
    soundKetThuc: cc.AudioClip = null;
    ...
    ping() {
        ...
        this.timeout += 5;
        MauBinhNetworkClient.getInstance().send(new Cmd.CmdSendPing());
    }
    ...
    playingNow() {
        ...
        for (let index = 0; index < this.contentListRooms.childrenCount; index++) {
            let roomItem = this.contentListRooms.children[index].getComponent(ItemRoom);
            console.log("tiền tối đa là :"+roomItem.roomInfo["requiredMoney"]);
        ...
        MauBinhNetworkClient.getInstance().addListener((data) => {
            ...
            switch (inpacket.getCmdId()) {
                case Cmd.Code.LOGIN:
                    ...
                    cc.log("MB Login res:", inpacket.getCmdId());
                    this.refeshListRoom();
                ...
                case Cmd.Code.GET_LIST_ROOM:
                    {
                        ...
                        cc.log(res);
                        res.list.sort((firstItem, secondItem) => (firstItem.moneyBet - secondItem.moneyBet));
                        cc.log(res.list);
                        for (let i = 0; i < res.list.length; i++) {
                            let itemData = res.list[i];
                            if(itemData["moneyBet"] == 5000)
                                continue;
                            let item = cc.instantiate(this.prefabItemRoom);
                            item.getComponent(ItemRoom).initItem(itemData);
                ...
                case Cmd.Code.BINH_SO_CHI:
                    {
                        App.instance.showLoading(false);
                        cc.audioEngine.play(this.soundTiengDong, false, 1);
                ...
                case Cmd.Code.KET_THUC:
                    {
                        ...
                        cc.audioEngine.play(this.soundKetThuc, false, 1);
                ...
                case Cmd.Code.CHIA_BAI:
                    {
                        ...
                        cc.audioEngine.play(this.soundChiaBai, false, 1);
