...
export default class Room extends cc.Component {
    ...
    ping() {
        ...
        this.timeout += 5;
        SamNetworkClient.getInstance().send(new SamCmd.SendPing());
    }
        ...
        SamNetworkClient.getInstance().addListener((data) => {
            ...
            switch (cmdId) {
                ...
                case SamCmd.Code.JOIN_ROOM_SUCCESS: {
                    ...
                    cc.log("Sam JOIN_ROOM_SUCCESS res : " , res);
                    cc.log("Vào đy");
                    this.show(false);
    ...
    initRooms(rooms) {
        ...
        for (let i = 0; i < rooms.length; i++) {
            ...
            item.getChildByName('lblRange').getComponent(cc.Label).string = `${Utils.numFormatter(room.moneyBet)} / ${Utils.numFormatter(room.moneyRequire)}`;
            item.getChildByName('lblNumPlayers').getComponent(cc.Label).string = `${room.nPersion} / ${room.maxUserPerRoom}`;
