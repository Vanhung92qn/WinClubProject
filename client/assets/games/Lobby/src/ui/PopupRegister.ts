    ...
    export class PopupRegister extends Popup {
        ...
        actionCheckNickName() {
            ...
            if (nickname && nickname.length > 6) {
                ...
                Http.get(Configs.App.API, reqParams, (err, res) => {
                    if (!res["success"]) {
                        App.instance.alertDialog.showMsg("Tên nhân vật đã tồn tại hoặc vừa được gán. Thử đăng nhập, hoặc chọn tên khác.");
                        ...
                        Http.get(Configs.App.API, reqParams, (err, res) => {
                            ...
                            if (!res["success"]) {
                                ...
                                if (String(res["errorCode"]) !== "0") {
                                    App.instance.alertDialog.showMsg("Phản hồi server không rõ: " + res["errorCode"]);
                                    return;
                                }
                                App.instance.actShowThongBao("Đã tạo tài khoản. Đang gán tên hiển thị…", 4);
                                App.instance.showLoading2(true);
                                const finishNickFlow = () => {
                                    this.runActionClose();
                                    _this.scheduleOnce(() => {
                                        _this.node.destroy();
                                    }, 0.45);
                                ...
                                Http.get(Configs.App.API, {
                                    ...
                                    App.instance.showLoading2(false);
                                    if (response && !response["success"] && String(response["errorCode"]) === "1013") {
                                        finishNickFlow();
                                        return;
                                    }
                                    const httpFail = err != null || response == null || err === "network_error" || err === "timeout"
                                        || (typeof err === "number" && err !== 200);
                                    if (httpFail) {
                                        App.instance.actShowThongBao("Mạng chậm hoặc mất phản hồi. Đang thử đăng nhập (nick có thể đã được gán)…", 5);
                                        finishNickFlow();
                                        return;
                                    }
                                    if (err != null) {
                                        App.instance.alertDialog.showMsg("Lỗi phản hồi server. Thử đăng nhập thủ công.");
                                        return;
                                    }
                                    if (!response["success"]) {
